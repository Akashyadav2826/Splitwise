...
export default function ExpenseFormPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setMembers(mems);
      setSplits(mems.map((m) => ({ userId: m.id, userName: m.name, value: 1 })));
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await api.post(`/groups/${groupId}/expenses`, {
        paidByUserId: paidBy, amount: parseFloat(amount),
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface-2)', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <Navbar />
        <div style={{ background: 'linear-gradient(135deg, #1a3a40 0%, #0d2a30 100%)', padding: '28px 0 60px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div className="page-container" style={{ paddingTop: 0, paddingBottom: 0, maxWidth: 640, position: 'relative', zIndex: 1 }}>
            ...
            <h2 style={{ fontSize: 26, fontWeight: 800, margin: '0 0 4px', color: 'white' }}>Add expense 💸</h2>
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.55)', margin: 0 }}>Record a shared expense and split it with your group</p>
          </div>
        </div>

        <div className="page-container" style={{ marginTop: -32, maxWidth: 640, position: 'relative', zIndex: 2 }}>
          {error && (
            <div style={{ background: '#fdecea', border: '1px solid #f5c6c2', borderRadius: 10, padding: '13px 16px', marginBottom: 16, fontSize: 14, color: '#c0392b' }}>⚠️ {error}</div>
          )}
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ background: 'white', borderRadius: 16, padding: 28, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                <div>
                  ...
                  <input className="input" type="text" value={description} onChange={(e) => setDescription(e.target.value)} required placeholder="e.g. Dinner at Pizza Hut, Hotel booking, Petrol" />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  <div>
                    ...
                    <select className="input" value={paidBy} onChange={(e) => setPaidBy(parseInt(e.target.value))} required>
            ...
            <div style={{ background: 'white', borderRadius: 16, padding: 28, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', border: '1px solid var(--border)' }}>
              ...
              {splitType === 'equal' ? (
                <div style={{ background: 'var(--brand-light)', borderRadius: 12, padding: '14px 18px', border: '1px solid #b2ede8', display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  {splits.map((s) => (
                    <div key={s.userId} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--surface-2)', borderRadius: 10, padding: '10px 14px', border: '1px solid var(--border)' }}>
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'var(--brand)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, flexShrink: 0 }}>{s.userName.charAt(0).toUpperCase()}</div>
                      ...
                      <input type="number" step={splitType === 'share' ? '1' : '0.01'} min="0"
                        ...
                        style={{ width: 110, border: '1.5px solid var(--border)', borderRadius: 8, padding: '8px 12px', fontSize: 15, fontWeight: 700, textAlign: 'right', outline: 'none', fontFamily: 'inherit', background: 'white' }}
